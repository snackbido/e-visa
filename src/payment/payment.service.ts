...
export class PaymentService {
  ...
  async initializePayment(paymentDto: PaymentDto): Promise<InitPaymentResult> {
    const date = Date.now();
    ...
    switch (payment_gate) {
      case 'ONEPAY':
        initPaymentResult = this.onePayService.init({
          ...
          orderInfo: payment.visa_id,
          amount: payment.amount,
          customerId: String(payment.user_id),
          // TODO: get customer ip
          customerIp: paymentDto.customer_ip as string,
  ...
  async verifyPayment(
    ...
    let verifyResult: { status: string; data?: any };

    switch (paymentGate) {
      case 'ONEPAY':
        verifyResult = await this.onePayService.verify(paymentId);
        break;
      default:
    ...
    if (!payment) {
      throw new NotFoundException(
        `Transaction with payment ID ${paymentId} not found`,
      );
    }
    ...
    switch (verifyResult.status) {
      ...
      case STATUS.EXPIRED:
        ...
        break;
      default:
        this.eventEmitter.emit('payment.fail', eventPayload);
        break;
    }
